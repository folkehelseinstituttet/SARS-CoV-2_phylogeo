---
title: "Results - B.1.1.7,  “UK variant”"
author: ""
date: "20/04/2021"
output: 
  github_document
...
#   ____________________________________________________________________________
#   Read required data                                                      ####
metadat = fread(paste0(path_Main,"nextstrain_groups_niph_ncov_UK-centric-2021-04-20_metadata.tsv"))
metadata = metadat
tree = read.tree(paste0(path_Main,"nextstrain_groups_niph_ncov_UK-centric-2021-04-20_timetree.nwk"))
...
nodelabels(cex=0.5)
tree = extract.clade(tree, node=6773)
...
transition_matrix = rbind(c(-0.07228033 ,0.07228033),c(0.72280328,-0.72280328)) #Write up from the print above
# #OLD WAS transition_matrix = rbind(c(-0.1127392 ,0.1127392),c(1.1273923,-1.1273923)) #Write up from the print above
...
## Run below command for new builds. If results needed fast use 5 replicates below, and rerun with e.g. 100.
#multi_counts = pbreplicate(5, LineageHomology_w_uncertainty(tree, ace_nodes = castor$ancestral_likelihoods, give_tips = Norwegian_tips,ace_tips = to.matrix(Locations,seq=c("Norway","RoW")),start_time = start_date))
...
### . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . ..
### Lineage size squareplots - not in report                                ####
...
g1 = ggplot(data=ggdata, aes(x=dates, y=fraction, fill=group)) + 
  ...
  ylab("")+xlab("")+
  annotate("rect",xmin=as.Date(decimal2Date(time_end-1/12)),xmax=as.Date(dat$dates[length(dat$dates)])-7,ymin=-Inf,ymax=Inf,alpha=0.2)+
...
g1 = ggplot(dat, aes(x=dates,y=i50))+geom_line(col="Red")+theme_minimal(base_size=20)+
  ...
  ylab("Import")+
  theme(legend.position = "none")+annotate("rect",xmin=as.Date(decimal2Date(time_end-1/12)),xmax=as.Date(dat$dates[length(dat$dates)])-7,ymin=-Inf,ymax=Inf,alpha=0.2)
g1
...
Perhaps the most interesting aspect of quantifying introductions, is to investigate the fate of individual inferred imports. Mandatory testing at the border and/or mandatory quaranteens etc will obviously have a bearing on the fate of newly introduced infections from abroad. The plot below visualizes the number of infections in Norway resulting from each individual import over time, stratified by inferred TMRCA of each introduced lineage. 
