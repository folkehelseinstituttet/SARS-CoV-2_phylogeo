...
knit: (function(input_file, encoding) {
 ...
 encoding=encoding,
 output_file=file.path(dirname(input_file), out_dir, 'Alpha.html'))})
---
...
metadat = fread(paste0(path_Main,"nextstrain_groups_niph_ncov_alpha-2021-08-08_metadata.tsv"))
metadata = metadat
tree = read.tree(paste0(path_Main,"nextstrain_groups_niph_ncov_alpha-2021-08-08_timetree.nwk"))
...
nodelabels(cex=0.5)
tree = extract.clade(tree, node=5928)
...
transition_matrix = rbind(c(-0.08, 0.08),c(0.8, -0.8)) # artificial rate as real estimated matrix failed in lineagehomology step. Estimated rate ca 0.07 and 0.7
## Old Estimate from 2021-08-02: transition_matrix = rbind(c(-0.1032237, 0.1032237),c(1.0322375, -1.0322375))
## Old Estimate from 2021-07-13: transition_matrix = rbind(c(-0.09335977, 0.09335977),c(0.93359768, -0.93359768))
## Old Estimate from 2021-06-21: transition_matrix = rbind(c(-0.1159497, 0.1159497),c(1.1594966, -1.1594966))
## Old Estimate from 2021-06-21: transition_matrix = rbind(c(-0.1159325, 0.1159325),c(1.1593248, -1.1593248))
...
g2 = ggplot(dat, aes(x=dates,y=e50))+geom_line(col="Darkgreen")+theme_minimal(base_size=20)+ylim(0,500)+
  scale_x_date(date_breaks="1 week",limits =c(as.Date("2020-11-30"),as.Date(dat$dates[length(dat$dates)])-7), expand=c(0,0))+
