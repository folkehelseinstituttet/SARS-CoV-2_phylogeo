---
title: "SARS-CoV-2 Omicron variant - 2022-01-22"
author: ""
output: 
  github_document
#pdf_document 
...
Phylogeographic analyses are only as good as the data we input. For these analyses we have assumed that there is a signal in the genetic data and that the time scaling of the phylogenetic tree is
Most of the first samples of Omicron in Norway came from a super-spreading event at Aker Brygge. There is a substantial overrepresentation of the outbreak in the genetic data, which could influence.  
...
The analyses take a dated phylogeny exported from [NextStrain](https://nextstrain.org/groups/niph/ncov/2022-01-17/omicron-BA-one) as a starting point for quantifying imports and local transmission of SARS-CoV-2 Omicron variant in Norway.The current set of sequences includes 678 Norwegian genome sequences, and 1345 from the "Rest of the world (RoW)"
...
metadat = fread(paste0(path_Main,"nextstrain_groups_niph_ncov_2022-01-17_omicron-BA-one_metadata.tsv"))
metadata = metadat #Used alternate spellings some places.
tree = read.tree(paste0(path_Main,"nextstrain_groups_niph_ncov_2022-01-17_omicron-BA-one_timetree.nwk"))
#   ____________________________________________________________________________
...
nodelabels(cex=0.5)
#tree = extract.clade(tree, node=2033) #No necessary here. 
...
#save(multi_counts, file=paste0(path_Data,"count_ML_Omicron_EqualRates_100.Rdata"))
load(paste0(path_Data,"count_ML_Omicron_EqualRates_100.Rdata"))
...
#save(Result,file=paste0(path_Data,"count_once_Omicron_EqualRates.Rdata"))
load(paste0(path_Data,"count_once_Omicron_EqualRates.Rdata"))
```
...
The SSE outbreak dominates the Norwegian sequences. If we exclude this lineage, the we obtain the following size distribution

```{r, echo=FALSE, fig.width=8, fig.height=8}
treemap2(Result,remove_largest=1)
#hist(sort(all_lineages,decreasing=T)[-1], breaks=20)
```

##### Fig. 3. Transmission lineage size as in Fig. 2, exluding the largest one.

The distribution of lineages is highly skewed with some . As can be seen from the color of the groups and TMRCA, most of the large lineages (orange and yellow) were introduced at earlier points in time, while the recenty introduced lineages (red) are still small. Bear in mind that these results are heavily influenced by the current set of available sequneces and are will change as the lineages grow and we obtain updated datasets.
...
lineage_growth_cumulative2 = function(Result_lineage_info,color_by_state=FALSE) {
  Result_lineage_info$group_cumsum = ave(Result_lineage_info$dates,as.factor(Result_lineage_info$group_no), FUN=function(x) rank(x,ties.method = "first"))-1 #Use the rank function to obtain cumulative counts. The -1 removes the count of the mrca.
    g1 = ggplot(Result_lineage_info,aes(x=decimal2Date(dates), y=group_cumsum,group=group_no))+geom_line(alpha=0.4)+theme_bw()+ylab("Group size")+xlab(element_blank())+
      scale_x_date(date_breaks="1 week",expand = c(0,0))+
      annotate("rect",xmin=as.Date("2021-12-13"),xmax=as.Date("2022-01-10"),ymin=-Inf,ymax=Inf,alpha=0.1)+
      theme(axis.text.x=element_text(angle=60, hjust=1))
  g1
}
...
The rapid growth of the SSE dominates the picture. After the initial event of spread the growth of this lineage continued, but in a slower fashion. This could also be a result of targeted sampling and sequencing of cases connected to this outbreak, followed by a less intense sampling regime of this afterwards. Or it could be actual, in that it first spread rapidly, and then slower in the community.  

If we exclude the known SSE, we obtain the following figure.
```{r,echo=F,fig.width=7, fig.height=7}
#Remove the largest lineage:
remove_inds = which(Result_lineage_info$group_no==1)
Result_lineage_info = Result_lineage_info[-remove_inds,]
...
mrca_sizes$halfedge_above_mrca =Result$Halfedge_over_tmrca
write.table(mrca_sizes, file="~/Dropbox/Covid/Omicron/Results/Omicron_lineage_sizes_EqualRates.txt")
write.csv(mrca_sizes, file="~/Dropbox/Covid/Omicron/Results/Omicron_lineage_sizes_EqualRates.csv")
