---
title: "Results - B.1.617.2, “Delta variant”"
author: ""
date: "13/07/2021"
output: 
...
#   ____________________________________________________________________________
#   Read required data                                                      ####
#   note, when data directly from HF included (not on GISAID) is included, replace tip labels
#   in input files, so that "Norway" label is included. This can be done with:
#   ~/Dropbox/Covid/B1-617-2/replace_words.py using the conversion table
#   ~/Dropbox/Covid/B1-617-2/conversion_HF_isolnames.csv
metadat = fread(paste0(path_Main,"nextstrain_groups_niph_ncov_B-1-617-2021-07-13_metadata_NN.tsv"))
metadata = metadat #Used alternate spellings some places.
tree = read.tree(paste0(path_Main,"nextstrain_groups_niph_ncov_B-1-617-2021-07-13_timetree_NN.nwk"))
...
nodelabels(cex=0.5)
#Prune away the part of the tree not corresponding to B.1.617.2. NB: update this for new builds.
tree = extract.clade(tree, node=2000)
...
# Run below for new builds - estimating transition matrix from whole B.1.617
castor = castor::asr_mk_model(tree,tip_states =locations_for_castor,
                              rate_model="ARD",include_ancestral_likelihoods = T)
castor$transition_matrix

#transition_matrix = rbind(c(-0.6985837, 0.6985837),c(0.2926757, -0.2926757))
...
#save(multi_counts, file=paste0(path_Data,"count_ML_B16172_100.Rdata"))
load(paste0(path_Data,"count_ML_B16172_100.Rdata"))
...
g1 = ggplot(dat, aes(x=dates,y=i50))+geom_line(col="Red")+theme_minimal(base_size=20)+
  ...
  scale_x_date(date_breaks="1 week",limits =c(as.Date("2021-04-01"), as.Date("2021-06-08")),expand=c(0,0))+
  ylab("Import")+
...
g2 = ggplot(dat, aes(x=dates,y=e50))+geom_line(col="Darkgreen")+theme_minimal(base_size=20)+ylim(0,30)+
  scale_x_date(date_breaks="1 week",limits =c(as.Date("2021-04-01"), as.Date("2021-06-08")), expand=c(0,0))+
...
g1 = ggplot(c2, aes(MRCA, Lineagesize))+
  ...
  annotate("rect",xmin=as.Date(decimal2Date(time_end-1/13)+7),xmax=as.Date(dat$dates[length(dat$dates)]+7),ymin=-Inf,ymax=Inf,alpha=0.2)
g1
...
write.table(mrca_sizes, file=("~/Dropbox/Covid/B1-617-2/MRCA_lineageSize_2021-07-13.txt"))
write.csv(mrca_sizes, file=("/home/vehuardo/Dropbox/Covid/Alpha_vs_Delta/delta.csv"))
