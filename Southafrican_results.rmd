...
#   ____________________________________________________________________________
#   Read required data                                                      ####
metadat = fread(paste0(path_Main,"nextstrain_groups_niph_ncov_southafrican-2021-04-14_metadata.tsv"))
metadata = metadat #Used alternate spellings some places.
tree = read.tree(paste0(path_Main,"nextstrain_groups_niph_ncov_southafrican-2021-04-14_timetree.nwk"))
...
tree = extract.clade(tree, node=1863)
plot.phylo(tree,show.tip.label = F)
...
#< multi_counts = replicate(100, LineageHomology_w_uncertainty(tree, ace_nodes = castor$ancestral_likelihoods, give_tips = Norwegian_tips,ace_tips = to.matrix(Locations,seq=c("Norway","RoW")),start_time = start_date))
...
g2 = ggplot(dat, aes(x=dates,y=e50))+geom_line(col="Darkgreen")+theme_minimal(base_size=20)+ylim(0,50)+
  scale_x_date(date_breaks="1 week",limits =c(as.Date("2020-11-30"), as.Date("2021-03-28")), expand=c(0,0))+
